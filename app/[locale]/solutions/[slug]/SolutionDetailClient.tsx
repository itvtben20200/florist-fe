...
function LeadForm({ accentColor, productName }: { accentColor: string; productName: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
      ...
      <div>
        <label style={labelStyle}>{t('formCompanyLabel')}</label>
        <input
          style={inputStyle}
          type="text"
      ...
      <div>
        <label style={labelStyle}>{t('formMessageLabel')}</label>
        <textarea
          style={{ ...inputStyle, resize: 'vertical', minHeight: '110px' }}
          placeholder={t('formMessagePlaceholder', { productName })}
          value={form.message}
...
export default function SolutionDetailClient({
  ...
  const t = useTranslations('SolutionDetail');
  const faqT = useTranslations('SolutionFaq');
  const contentT = useTranslations('SolutionContent');
  ...
  const addOnProducts = useMemo(() => {
    ...
    return products.flatMap((p) => {
      ...
      const addOnSlugKey = canonicalSlug.replace(/-([a-z])/g, (_: string, c: string) => c.toUpperCase());
      return [{
        ...p,
        canonicalSlug,
        displayName: solutionContent?.name ?? p.name,
        displayImage: solutionContent?.heroImage ?? p.images?.[0],
        moduleTagline: solutionContent ? contentT(`${addOnSlugKey}_tagline` as Parameters<typeof contentT>[0]) : undefined,
        moduleOverview: solutionContent ? contentT(`${addOnSlugKey}_overview` as Parameters<typeof contentT>[0]) : undefined,
        moduleBenefits: solutionContent?.benefits?.slice(0, 3).map((_, i) => contentT(`${addOnSlugKey}_benefit_${i}_title` as Parameters<typeof contentT>[0])) ?? [],
      }];
    });
  ...
  return (
    <div style={{ background: '#fafaf8', ...mono }}>
      ...
      >
        <Link href="/" style={{ color: '#aaa', textDecoration: 'none' }}>{t('breadcrumbHome')}</Link>
          ...
          <div className={`grid gap-4 ${isFloristCore ? 'lg:grid-cols-2' : 'lg:grid-cols-1'}`}>
            ...
            >
              {price !== null ? (
                <div style={{ marginBottom: '20px' }}>
                  <div className="flex items-end gap-3 mb-1">
                    ...
                    <div style={{ paddingBottom: '8px', display: 'flex', flexDirection: 'column', gap: '1px' }}>
                      <span style={{ ...mono, fontSize: '11px', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', color: '#888' }}>
                        {t('perMonthPerLicence')}
                      </span>
                    </div>
                  </div>
                  <p style={{ ...mono, fontSize: '11px', color: '#bbb', margin: 0, letterSpacing: '0.3px' }}>
                    {t('billingNote')}
                  </p>
                </div>
              ) : (
                <div style={{ ...mono, fontSize: '14px', color: '#aaa', marginBottom: '20px' }}>
              ...
              {isFloristCore && selectedAddOns.length > 0 && (
                <p style={{ ...mono, fontSize: '11px', color: '#777', margin: '0 0 14px' }}>
                  {selectedAddOns.length === 1
                    ? t('addOnSelected', { count: selectedAddOns.length, price: addOnsPrice.toFixed(2) })
                    : t('addOnsSelected', { count: selectedAddOns.length, price: addOnsPrice.toFixed(2) })}
                </p>
              )}
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                >
                  {outOfStock ? t('btnUnavailable') : t('btnGetStarted')}
                </button>
                ...
                >
                  {added ? t('btnAdded') : t('btnAddToCart')}
                </button>
              </div>

              <div className="flex items-center gap-4 mt-5 flex-wrap">
                {[
                  { icon: '🔒', text: t('trustSecureCheckout') },
                  { icon: '💬', text: t('trustExpertSupport') },
                  { icon: '📞', text: t('trustOnboarding') },
                ].map(({ icon, text }) => (
            ...
            >
              ...
              <div style={{ padding: '24px 24px 26px' }}>
                ...
                <p style={{ ...mono, fontSize: '13px', color: '#555', lineHeight: 1.75, marginBottom: '15px' }}>
                  {activeModule.moduleOverview ?? t('moduleDefaultOverview')}
                </p>
            ...
            {content.benefits.map((b, i) => (
              ...
              >
                ...
                <h3 style={{ ...serif, fontSize: '20px', fontWeight: 500, color: '#1a1a1a', marginBottom: '8px' }}>
                  {contentT(`${slugKey}_benefit_${i}_title` as Parameters<typeof contentT>[0])}
                </h3>
                <p style={{ ...mono, fontSize: '12px', color: '#777', lineHeight: 1.7, margin: 0 }}>
                  {contentT(`${slugKey}_benefit_${i}_body` as Parameters<typeof contentT>[0])}
                </p>
              </div>
            ))}
          </div>
              ...
              {content.inclusions.map((inc, i) => (
                ...
                >
                  ...
                  <span style={{ ...mono, fontSize: '12px', color: '#333', fontWeight: 500 }}>
                    {contentT(`${slugKey}_inclusion_${i}` as Parameters<typeof contentT>[0])}
                  </span>
                  <span
                    style={{
                      ...mono,
            ...
            {content.useCases.map((uc, i) => (
              ...
              >
                ...
                <h3 style={{ ...serif, fontSize: '22px', fontWeight: 500, color: 'white', marginBottom: '10px', lineHeight: 1.1 }}>
                  {contentT(`${slugKey}_useCase_${i}_title` as Parameters<typeof contentT>[0])}
                </h3>
                <p style={{ ...mono, fontSize: '12px', color: 'rgba(255,255,255,0.75)', lineHeight: 1.75, margin: 0 }}>
                  {contentT(`${slugKey}_useCase_${i}_desc` as Parameters<typeof contentT>[0])}
                </p>
              </div>
            ))}
          </div>
              ...
              >
                ...
                >
                  <span style={{ ...mono, fontSize: '13px', fontWeight: 600, color: '#1a1a1a' }}>
                    {faqT(`${faqSlugKey}_q${idx}` as Parameters<typeof faqT>[0])}
                  </span>
                  <svg
                    width="16"
                    height="16"
                ...
                {openFaq === idx && (
                  <div style={{ padding: '0 22px 18px', borderTop: '1px solid #f5f2ee' }}>
                    <p style={{ ...mono, fontSize: '12px', color: '#666', lineHeight: 1.75, margin: 0 }}>
                      {faqT(`${faqSlugKey}_a${idx}` as Parameters<typeof faqT>[0])}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-start">
            ...
            <div className="w-full lg:w-[42%] flex-shrink-0 lg:sticky lg:top-24">
              ...
              <div className="flex flex-col gap-3 mt-8">
                {[
                  { icon: '💬', label: t('contactTrust1') },
                  { icon: '🔒', label: t('contactTrust2') },
                  { icon: '📞', label: t('contactTrust3') },
                ].map(({ icon, label }) => (
