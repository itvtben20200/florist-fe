...
const INTEG_CARDS = [
  { icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4', color: GREEN, bg: '#f0fdf5' },
  { icon: 'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 11h.01M15 11h.01M9 17h6M5 3h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2z', color: '#1B3A6D', bg: '#f0f4fc' },
  { icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z', color: '#7c3aed', bg: '#f5f0fe' },
  { icon: 'M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z', color: '#0e6b4a', bg: '#f0fdf5' },
];
...
export default async function PosTerminalPage() {
  ...
  const PAINS_WITH_COPY = PAINS.map((pain, index) => ({
    ...pain,
    title: t(`pain${index}title`),
    body: t(`pain${index}body`),
  }));
  ...
  const STEPS = STEP_ICONS.map((icon, index) => ({
    ...
    body: t(`step${index}body`),
  }));

  const INTEG_CARDS_WITH_COPY = INTEG_CARDS.map((card, index) => ({
    ...card,
    title: t(`integ${index}title`),
    body: t(`integ${index}body`),
  }));

  const USE_CASES = [0, 1, 2, 3].map((index) => ({
    title: t(`uc${index}title`),
    description: t(`uc${index}body`),
  }));

  const FAQ_ITEMS = [0, 1, 2, 3, 4].map((index) => ({
    question: t(`faq${index}q`),
    answer: t(`faq${index}a`),
  }));
  ...
  const SALES_BREAKDOWN = [
    { label: t('adv2item0'), pct: 85, value: 'EUR 1,240' },
    { label: t('adv2item1'), pct: 60, value: 'EUR 870' },
    { label: t('adv2item2'), pct: 40, value: 'EUR 580' },
    { label: t('adv2item3'), pct: 25, value: 'EUR 360' },
  ];
  ...
  return (
    <div style={{ background: '#fafaf8', ...mono }}>
      ...
      <section style={{ background: DARK_GREEN, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'url(/solutions/pos-terminal.svg)', backgroundSize: 'cover', backgroundPosition: 'center right', opacity: 0.18, pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(to right, rgba(13,64,48,1) 40%, rgba(13,64,48,0.6) 100%)', pointerEvents: 'none' }} />
      ...
      <section className="max-w-screen-xl mx-auto px-4 sm:px-[5%] py-16 sm:py-24">
        <div className="flex flex-col lg:flex-row gap-14 items-start">
          <div className="w-full lg:w-[44%] flex-shrink-0">
            <span style={{ ...mono, fontSize: '10px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: ROSE }}>{t('challengeLabel')}</span>
      ...
      <section id="how-it-works" style={{ background: DARK_GREEN }}>
        <div className="max-w-screen-xl mx-auto px-4 sm:px-[5%] py-16 sm:py-24">
          <div style={{ textAlign: 'center', marginBottom: '56px' }}>
            <span style={{ ...mono, fontSize: '10px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.45)' }}>{t('howLabel')}</span>
            <h2 style={{ ...serif, fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 500, color: 'white', marginTop: '10px', lineHeight: 1.1 }}>{t('howTitle')}</h2>
          </div>
      ...
      <section className="max-w-screen-xl mx-auto px-4 sm:px-[5%] py-16 sm:py-24">
        <div style={{ marginBottom: '48px' }}>
          <span style={{ ...mono, fontSize: '10px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: ROSE }}>{t('whyLabel')}</span>
          <h2 style={{ ...serif, fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 500, color: '#1a1a1a', marginTop: '10px', lineHeight: 1.1 }}>{t('whyTitle')}</h2>
        </div>
        <div className="flex flex-col gap-6">
          ...
          <div style={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid #f0ede8' }} className="grid grid-cols-1 lg:grid-cols-2">
            <div style={{ background: '#f5fdf8', padding: '44px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              <h3 style={{ ...serif, fontSize: 'clamp(24px, 2.5vw, 34px)', fontWeight: 500, color: '#1a1a1a', marginBottom: '16px', lineHeight: 1.15 }}>{t('adv0title')}</h3>
              <p style={{ ...mono, fontSize: '13px', color: '#555', lineHeight: 1.85, marginBottom: '24px' }}>{t('adv0body')}</p>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {[t('adv0pt0'), t('adv0pt1'), t('adv0pt2')].map((pt) => (
            ...
            <div style={{ background: GREEN, padding: '44px 40px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '260px' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ ...serif, fontSize: '64px', fontWeight: 500, color: 'rgba(255,255,255,0.12)', lineHeight: 1 }}>3Ã—</div>
          ...
          <div style={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid #f0ede8' }} className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div style={{ background: 'white', padding: '44px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }} className="order-first lg:order-last">
              ...
              <h3 style={{ ...serif, fontSize: 'clamp(24px, 2.5vw, 34px)', fontWeight: 500, color: '#1a1a1a', marginBottom: '16px', lineHeight: 1.15 }}>{t('adv1title')}</h3>
              <p style={{ ...mono, fontSize: '13px', color: '#555', lineHeight: 1.85, marginBottom: '24px' }}>{t('adv1body')}</p>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {[t('adv1pt0'), t('adv1pt1'), t('adv1pt2')].map((pt) => (
                  <li key={pt} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
          ...
          <div style={{ borderRadius: '20px', overflow: 'hidden', border: '1px solid #f0ede8' }} className="grid grid-cols-1 lg:grid-cols-2">
            <div style={{ background: '#fdf9f6', padding: '44px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              <h3 style={{ ...serif, fontSize: 'clamp(24px, 2.5vw, 34px)', fontWeight: 500, color: '#1a1a1a', marginBottom: '16px', lineHeight: 1.15 }}>{t('adv2title')}</h3>
              <p style={{ ...mono, fontSize: '13px', color: '#555', lineHeight: 1.85, marginBottom: '24px' }}>{t('adv2body')}</p>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {[t('adv2pt0'), t('adv2pt1'), t('adv2pt2')].map((pt) => (
                  <li key={pt} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
            ...
            <div style={{ background: '#fdf2f4', padding: '44px 40px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '260px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '100%', maxWidth: '280px' }}>
                ...
                <div style={{ ...mono, fontSize: '10px', color: '#bbb', marginTop: '4px', fontWeight: 500, letterSpacing: '0.5px' }}>{t('adv2chart')}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ background: 'white', borderTop: '1px solid #f0ede8', borderBottom: '1px solid #f0ede8' }}>
        <div className="max-w-screen-xl mx-auto px-4 sm:px-[5%] py-16 sm:py-24">
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <span style={{ ...mono, fontSize: '10px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: ROSE }}>{t('integLabel')}</span>
            <h2 style={{ ...serif, fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 500, color: '#1a1a1a', marginTop: '10px', lineHeight: 1.1 }}>{t('integTitle')}</h2>
            <p style={{ ...mono, fontSize: '14px', color: '#777', lineHeight: 1.8, marginTop: '14px', maxWidth: '520px', marginLeft: 'auto', marginRight: 'auto' }}>{t('integBody')}</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(260px, 100%), 1fr))', gap: '20px' }}>
            {INTEG_CARDS_WITH_COPY.map((card) => (
      ...
      <section className="max-w-screen-xl mx-auto px-4 sm:px-[5%] py-16 sm:py-24">
        <div style={{ marginBottom: '48px' }}>
          <span style={{ ...mono, fontSize: '10px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: ROSE }}>{t('whoLabel')}</span>
          <h2 style={{ ...serif, fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 500, color: '#1a1a1a', marginTop: '10px', lineHeight: 1.1 }}>{t('whoTitle')}</h2>
        </div>
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-[5%] py-16 sm:py-20">
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
            <div className="w-full lg:w-[36%] flex-shrink-0 lg:sticky lg:top-24">
              <span style={{ ...mono, fontSize: '10px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: ROSE }}>{t('faqLabel')}</span>
              <h2 style={{ ...serif, fontSize: 'clamp(26px, 3vw, 38px)', fontWeight: 500, color: '#1a1a1a', marginTop: '10px', lineHeight: 1.12 }}>{t('faqTitle')}</h2>
              <p style={{ ...mono, fontSize: '13px', color: '#777', lineHeight: 1.75, marginTop: '14px' }}>{t('faqBody')}</p>
            </div>
      ...
      <section style={{ background: DARK_GREEN }}>
        <div className="max-w-screen-xl mx-auto px-4 sm:px-[5%] py-16 sm:py-20 flex flex-col sm:flex-row items-center justify-between gap-8">
          <div>
            <h2 style={{ ...serif, fontSize: 'clamp(26px, 3vw, 38px)', fontWeight: 500, color: 'white', lineHeight: 1.15, marginBottom: '10px' }}>{t('ctaTitle')}</h2>
            <p style={{ ...mono, fontSize: '13px', color: 'rgba(255,255,255,0.6)', lineHeight: 1.7, margin: 0 }}>{t('ctaBody')}</p>
          </div>
