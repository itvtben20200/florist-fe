...
export default async function HomePage() {
  ...
  // Build translated copy map inside the function so locale is respected
  const sc = await getTranslations('SolutionCopy');
  const solutionCopyMap: Record<string, SolutionCopy> = {
    'florist core':                          { title: sc('floristCoreTitle'),    description: sc('floristCoreDesc') },
    'daily close agent':                     { title: sc('dailyCloseTitle'),     description: sc('dailyCloseDesc') },
    'monthly close agent':                   { title: sc('monthlyCloseTitle'),   description: sc('monthlyCloseDesc') },
    'quarterly close agent':                 { title: sc('quarterlyCloseTitle'), description: sc('quarterlyCloseDesc') },
    'yearly close agent':                    { title: sc('yearlyCloseTitle'),    description: sc('yearlyCloseDesc') },
    'managed secured workplace':             { title: sc('workplaceTitle'),      description: sc('workplaceDesc') },
  ...
  const productsWithVisuals = products.flatMap((product) => {
    ...
    return [{
      ...product,
      slug: canonicalSlug,
      name: mappedCopy?.title ?? solutionContent.name,
          ...
          >
            <div className="mb-10 flex flex-col gap-6 text-left md:mb-12 md:flex-row md:items-end md:justify-between">
              <div className="max-w-2xl">
                ...
                >
                  {t('Products.headingSubtitle')}
                </p>
              </div>

              <div className="flex flex-wrap gap-2.5 md:max-w-[320px] md:justify-end">
                {[t('Products.pill1'), t('Products.pill2'), t('Products.pill3')].map((pill) => (
                  <span
                    key={pill}
