...
function downloadReceipt(order: Order) {
  import('jspdf').then(({ jsPDF }) => {
    ...
    doc.text('Thank you for your purchase. Florist — floriststore.com', 20, 280);
    doc.save(`Florist-Receipt-${ref}.pdf`);
  });
}
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          {orders.length !== 1 ? t('ordersFoundPlural', { count: orders.length }) : t('ordersFound', { count: orders.length })}
        </p>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="bg-white border rounded-xl p-10 text-center">
          ...
          <p className="font-medium mb-1">{t('noOrdersYet')}</p>
          <p className="text-sm text-gray-500 mb-5">{t('noOrdersDesc')}</p>
                  ...
                  <div className="border-t px-5 py-4 bg-gray-50 space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400">
                        {t('placedOn')} {new Date(order.createdAt).toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' })}
                      </span>
                    </div>
                      ...
                      <table className="w-full text-sm min-w-[400px]">
                        <thead>
                          <tr className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
                            <th className="text-left px-4 py-2.5">{t('product')}</th>
                            <th className="text-center px-4 py-2.5">{t('qty')}</th>
                            <th className="text-right px-4 py-2.5">{t('unitPrice')}</th>
                            <th className="text-right px-4 py-2.5">{t('total')}</th>
                          </tr>
                        </thead>
                        ...
                        <tfoot className="border-t bg-gray-50 text-sm">
                          <tr>
                            <td colSpan={3} className="px-4 py-2 text-right text-gray-500">{t('subtotal')}</td>
                            <td className="px-4 py-2 text-right">€{Number(order.subtotal).toFixed(2)}</td>
                          </tr>
                          <tr>
                            <td colSpan={3} className="px-4 py-1 text-right text-gray-500">{t('shipping')}</td>
                            <td className="px-4 py-1 text-right">€{Number(order.shippingCost ?? 0).toFixed(2)}</td>
                          </tr>
                          <tr className="font-bold">
                            <td colSpan={3} className="px-4 py-2.5 text-right">{t('total')}</td>
