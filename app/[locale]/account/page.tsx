  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-black text-white rounded-xl px-6 py-5 flex items-center justify-between">
        <div>
          <p className="text-white/60 text-sm mb-1">{t('welcomeBack')}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { href: '/account/orders', icon: '📦', label: t('myOrders') },
          { href: '/account/profile', icon: '✏️', label: t('editProfile') },
          { href: '/account/security', icon: '🔒', label: t('changePassword') },
          { href: '/', icon: '🛍️', label: t('shopAgain') },
        ].map((q) => (
          <Link
            key={q.href}
            href={q.href}
      ...
      {recent.length > 0 && (
        <div className="bg-white border rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b">
            ...
            <Link href="/account/orders" className="text-xs underline text-gray-500">{t('viewAll')}</Link>
          </div>
